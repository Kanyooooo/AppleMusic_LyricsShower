    ...
    public bool TryGet(TrackInfo track, out LyricsCacheResult result)
    {
        lock (_lock)
        {
            ...
            trackCommand.CommandText = """
                SELECT title, artist, album, duration_ms, source, is_synced, fingerprint, captured_at_utc
                FROM tracks
            ...
            {
                result = new LyricsCacheResult(LyricsBundle.Empty("No cached lyric lines"), false, string.Empty);
                return false;
            }
    ...
    public void Save(TrackInfo track, LyricsBundle lyrics)
    {
        ...
        lock (_lock)
        {
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                command.CommandText = """
                    INSERT INTO tracks (
                        track_key, title, artist, album, duration_ms, source, is_synced, fingerprint, captured_at_utc
                    )
                    VALUES (
                        $track_key, $title, $artist, $album, $duration_ms, $source, $is_synced, $fingerprint, $captured_at_utc
                    )
