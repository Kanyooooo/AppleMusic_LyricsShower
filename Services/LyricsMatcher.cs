...
{
    public const double MinimumAnchoredConfidence = 0.58;
    public const double MinimumUnanchoredConfidence = 0.70;
        ...
        IReadOnlyCollection<string>? visibleAnchors = null)
    {
        ...
        var best = candidates
            ...
            .Select(candidate =>
            {
                ...
                var score = Score(candidate, track, anchorHits, visibleAnchors);
                return new LyricMatch(
                    candidate,
                    score,
                    anchorHits,
                    ConfidenceFromScore(score, anchorHits, visibleAnchors.Count));
            })
        ...
        if (hasAnchors)
        {
            return best[0].Confidence < MinimumAnchoredConfidence ? null : best[0];
        }

        if (best[0].Confidence < MinimumUnanchoredConfidence)
        {
            return null;
        }
        ...
        IReadOnlyCollection<string>? visibleAnchors = null)
    {
        ...
        var anchorHits = CountAnchorHits(lyrics, visibleAnchors);
        var score = Score(lyrics, track, anchorHits, visibleAnchors);
        return ConfidenceFromScore(score, anchorHits, visibleAnchors.Count);
    }
    ...
    private static double ConfidenceFromScore(double score, int anchorHits, int anchorCount)
    {
        if (anchorCount > 0)
        {
            var anchorRatio = Math.Min(1.0, (double)anchorHits / Math.Min(anchorCount, 6));
            var scoreConfidence = Clamp((score - 40) / 1600);
            return Clamp(0.30 + anchorRatio * 0.50 + scoreConfidence * 0.20);
        }
